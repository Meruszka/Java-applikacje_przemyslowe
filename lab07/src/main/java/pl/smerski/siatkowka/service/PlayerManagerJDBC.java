...
public class PlayerManagerJDBC implements PlayerManager{
    ...
    public Player updatePlayer(Player player) throws Exception {
        jdbcTemplate.update("UPDATE players SET name=?, surname=?, age=?, height=?, team=? WHERE id=?",
                player.getName(), player.getSurname(), player.getAge(), player.getHeight(), player.getTeam(), player.getId());
        return player;
    }
